    ...
    <a-form :model="formState" name="custom-validation" :rules="rules" :labelCol="{ lg: { span: 7 }, sm: { span: 7 } }"
      ...
      <a-form-item label="投票信息" :rules="[{ required: true }]">
        ...
        <a-table :columns="columns" :data-source="formState.data" :pagination="mypagination" size="large"
          ...
          <template #title>链上选票信息</template>
      ...
      <a-form-item label="本地信息" has-feedback :rules="[{ required: true }]">
      ...
      <a-form-item label="秘密份额" :rules="[{ required: true }]">
        <a-descriptions bordered="true" :column="{ xxl: 1, xl: 1, lg: 1, md: 1, sm: 1, xs: 1 }">
          <a-descriptions-item label="匿名灵活投票秘密份额">{{ formState.share }}</a-descriptions-item>
        </a-descriptions>
      </a-form-item>
      ...
      <div class="step-form-style-desc">
        <h3>说明</h3>
        <h4>加入匿名灵活信息意见表决事项：请根据以上项目ID返回投票信息完成投票表决。</h4>
      </div>
    </a-form>
  </div>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    addshare() {
      this.$http.post('http://192.168.5.42:9999/voteravc/upload_share', {
